...
codeForm.addEventListener("submit", async (event) => {
  ...
  // Validar que el usuario y el código existan
  ...
  try {
    const response = await fetch("funtions/guardar-cliente.php", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ usuario: username, clave: code }),
    });

    const result = await response.json();

    if (response.ok) {
      // Redirigir al usuario a espera.php con el ID en la URL
      const clienteId = result.clienteId; // ID devuelto por el servidor
